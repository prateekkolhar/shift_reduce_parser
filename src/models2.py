...
def train_beamed_model(parsed_sentences, feature_indexer, gold_state_cache, epoch_num=10, beam_size=3, dev=-1, test=-1):
    ...
    for epochs in xrange(epoch_num):
        print "epochs:" +str(epochs)
        for sentence_idx in xrange(0, len(parsed_sentences)):
#             print "sentence:"+str(sentence_idx)
...
#                     print "prev_beam_idx",prev_elt_idx
                    ...
                    for action in ["S","L","R"]:
                        take_action=False
                        if action=="S" and prev_state.buffer_len()!=0:
                            take_action=True
                        elif action=="L" and len(prev_state.stack)>=2 and prev_state.stack_two_back() !=-1:
                            take_action=True
                        elif action=="R" and len(prev_state.stack)>=2:
                            take_action=True
                        
                        if take_action:
                            (new_state,new_score,new_cum_f, new_is_gold) = compute_next_state(action)

                            if(new_is_gold): 
                                gold_cum_f = new_cum_f
                                
                            gold_in_beam &= b_c.add((new_state,new_cum_f), new_score, new_is_gold)
                b_p = b_c
                b_c = Beam(beam_size)
                if(not gold_in_beam): 
            ...
            pred_cum_f =b_p.elts[0][1]
            
            for idx in pred_cum_f.keys():
                wt[idx]-=pred_cum_f.get_count(idx)
                wt_a[idx]-=c*pred_cum_f.get_count(idx)
            for idx in gold_cum_f.keys():
                wt[idx]+=gold_cum_f.get_count(idx)
                wt_a[idx]+=c*gold_cum_f.get_count(idx)
            c+=1
    ...
    stop = timeit.default_timer()
    print "train complete:" + str(stop-start)
    return BeamedModel(feature_indexer, wt-wt_a/c,beam_size)
...
def extract_features(feat_indexer, sentence, parser_state, decision, add_to_indexer):
    ...
    add_feat(decision+":s0w-s1wp="+stk[0].word+"&"+stk[1].word+"&"+stk[1].pos)
    add_feat(decision+":s0p-s1wp="+stk[0].pos+"&"+stk[1].word+"&"+stk[1].pos)
    add_feat(decision+":s0wp-s1w="+stk[0].word+"&"+stk[0].pos+"&"+stk[1].word)
