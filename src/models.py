...
class GreedyModel(object):
    ...
    # the predicted parse.
    def parse(self, sentence, feature_cache=-1, decision_state_cache=-1, label_indexer = -1):
        
        pred = np.zeros([len(decision_state_cache[0]),len(label_indexer)])
        
        if feature_cache==-1 or decision_state_cache==-1 or label_indexer == -1:
            decision_state_cache = get_decision_sequence(sentence)
            label_indexer = get_label_indexer()
            feature_cache = [[[] for k in xrange(0, len(label_indexer))] for j in xrange(0, len(decision_state_cache[0]))] 
            
            for state_idx in xrange(0,len(decision_state_cache[0])):
                for label_idx in xrange(0,len(label_indexer)):
                    feature_cache[state_idx][label_idx] = extract_features(
                        self.feature_indexer,sentence, decision_state_cache[1][state_idx], label_indexer.get_object(label_idx), True)
                    pred[state_idx][label_idx] = self.feature_weights(feature_cache[state_idx][label_idx])
        else:
            for state_idx in xrange(0,len(decision_state_cache[0])):
                for label_idx in xrange(0,len(label_indexer)):
                    pred[state_idx][label_idx] = self.feature_weights(feature_cache[state_idx][label_idx])
        
        pred_final = pred.argmax(1)
        parser = initial_parser_state(len(sentence))
        for i in xrange(len(decision_state_cache[0])):
            parser.take_action(label_indexer.get_object(pred_final[i]))
        
        return ParsedSentence(sentence.tokens, parser.deps)
...
def generate_features(parsed_sentences, add_to_indexer):
    feature_indexer = Indexer();
    label_indexer = get_label_indexer();
    
    decision_state_cache = [get_decision_sequence(sentence) for sentence in parsed_sentences];
...
def train_greedy_model(parsed_sentences, feature_indexer=-1, label_indexer=-1, decision_state_cache=-1, feature_cache=-1 ):
    ...
    if feature_indexer==-1:
        ...
        print "train features extraction"
        start = timeit.default_timer()
        for sentence_idx in xrange(0, len(parsed_sentences)):
            if sentence_idx%100 == 0: print str(sentence_idx)+"/"+ str(len(parsed_sentences))
            for state_idx in xrange(0,len(decision_state_cache[sentence_idx][0])):
                for label_idx in xrange(0,len(label_indexer)):
                    feature_cache[sentence_idx][state_idx][label_idx] = extract_features(
                        feature_indexer,parsed_sentences[sentence_idx], decision_state_cache[sentence_idx][1][state_idx], label_indexer.get_object(label_idx), True)
        
        
        stop = timeit.default_timer()
        print "train features extracted:" + str(stop-start)
    wt = np.zeros(len(feature_indexer))
    print "Train"
    
    start = timeit.default_timer()
    for epochs in xrange(10):
        ...
        for sentence_idx in xrange(0, len(parsed_sentences)):
            for state_idx in xrange(0,len(decision_state_cache[sentence_idx][1])):
                ...
                for label_idx  in xrange(0,len(label_indexer)):
                    ...
                    wt[feature_cache[sentence_idx][state_idx][label_idx]] = \
                       wt[feature_cache[sentence_idx][state_idx][label_idx]] - pred[label_idx] 
                       
                wt[feature_cache[sentence_idx][state_idx][label_indexer.getindex(decision_state_cache[sentence_idx][0][state_idx])]] = \
                   wt[feature_cache[sentence_idx][state_idx][label_indexer.getindex(decision_state_cache[sentence_idx][0][state_idx])]] +  1;
                
        

    stop = timeit.default_timer()
    print "train complete:" + str(stop-start)
    return GreedyModel(feature_indexer, wt);
